...
char BaseRubiksCube::getColorLetter(COLOR color) {
    switch (color) {
        case COLOR::BLUE:
            return 'B';
        case COLOR::GREEN:
...
string BaseRubiksCube::getMove(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return "L";
        case MOVE::LPRIME:
...
BaseRubiksCube &BaseRubiksCube::move(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return this->l();
        case MOVE::LPRIME:
...
BaseRubiksCube &BaseRubiksCube::invert(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return this->lPrime();
        case MOVE::LPRIME:
...
void BaseRubiksCube::print() const {
    cout << "Rubik's Cube:\n\n";
...
vector<BaseRubiksCube::MOVE> BaseRubiksCube::randomShuffleCube(unsigned int times) {
...
string BaseRubiksCube::getCornerColorString(uint8_t ind) const {
    string str = "";
...
uint8_t BaseRubiksCube::getCornerIndex(uint8_t ind) const {
...
uint8_t BaseRubiksCube::getCornerOrientation(uint8_t ind) const {
    string corner = getCornerColorString(ind);
    string actual_str = "";
    for (auto c: corner) {
        if (c != 'W' && c != 'Y') continue;
