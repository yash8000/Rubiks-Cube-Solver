...
private:
    ...
    unordered_map<T, BaseRubiksCube::MOVE, H> move_done;
...
//        priority_queue contains pair(Node, move done to reach that)
        ...
        while (!pq.empty()) {
            ...
            move_done[node.cube] = BaseRubiksCube::MOVE(p.second);

            if (node.cube.isSolved()) return make_pair(node.cube, bound);
            node.depth++;
            for (int i = 0; i < 18; i++) {
                auto curr_move = BaseRubiksCube::MOVE(i);
...
public:
    ...
    vector<BaseRubiksCube::MOVE> solve() {
        ...
        while (!(curr_cube == rubiksCube)) {
            BaseRubiksCube::MOVE curr_move = move_done[curr_cube];
