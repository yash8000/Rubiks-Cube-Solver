...
public:
    ...
    virtual uint32_t getDatabaseIndex(const BaseRubiksCube &cube) const = 0;

    virtual bool setNumMoves(const BaseRubiksCube &cube, const uint8_t numMoves);

    virtual bool setNumMoves(const uint32_t ind, const uint8_t numMoves);

    virtual uint8_t getNumMoves(const BaseRubiksCube &cube) const;
